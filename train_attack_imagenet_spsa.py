...
'''
python train_attack_imagenet_spsa.py --device cuda:0 --checkpoint imagenet1k_resnet50 --tag spsa/00
'''
...
args = cfg.load(*args['config'], cfg_dir='train_attack_imagenet', args=args)
...
if args.images_per_class != -1:
    labels = torch.load('./data/imagenet1k_labels.pt')
    labels = {key: sorted(item) for key, item in labels.items()}
    indices = sum([item[:args.images_per_class] for _, item in labels.items()], [])
    trainset = Subset(trainset, indices)

loader_kwargs = dict(num_workers=14, 
...
# Model
print('==> BUILD MODEL')
model = models.get_model(checkpoint['model'], num_classes=1000).eval().to(DEVICE)
        ...
        for epoch in epoch_bar:
            ...
            with tqdm(trainloader, desc='TRAIN', position=2, leave=False, total=max_iters) as train_bar:
                ...
                for batch_idx, (inputs, targets) in enumerate(train_bar):
                    ...
                    u = (torch.rand_like(uap, device=DEVICE) > 0.5) * 2.0 - 1.0
                    u_beta = u * beta_scheduler.value
                    # END
                    ...
                    if len(accm_outputs_neg) == args.accumulation:
                        ...
                        # BEGIN
                        uap.grad = ((fool_neg - fool_pos) / (beta_scheduler.value * 2.0)) / u
                        # END
                        optimizer.step()
