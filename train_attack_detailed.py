...
'''
python train_attack_detailed.py --device cuda:1 -c cifar10-optim/adam --checkpoint cifar10_resnet18_1.0 -eipc 1 --eval-batch-size 10 --epochs 3000 --tag exp_n-eval_10
'''
...
captures_filename = exp_dir.joinpath('captures_{:04d}.pth')
...
sample_indices = torch.tensor([1, 5, 9]).long()
...
def batch_loss():
    ...
    with tqdm(range(1, args.epochs + 1), desc='EPOCH', position=1, leave=False) as epoch_bar:
        ...
        for epoch in epoch_bar:
            ...
            with tqdm(trainloader, desc='TRAIN', position=2, leave=False, total=max_iters) as train_bar:
                ...
                for batch_idx, (inputs, targets) in enumerate(train_bar):
                    ...
                    if len(accm_outputs_p0) == args.accumulation:
                        ...
                        now = time()
                        captures.append((uap.clone().detach().cpu(), now - start_time, num_updates))
                        torch.save(captures, captures_filename)
            ...
            torch.save(captures, str(captures_filename).format(epoch))
            captures.clear()
            if fr >= 100.0:
                _print(f'Achieved {fr:.3f}% at {epoch} epochs.')
                _print('Terminate training.')
                break
